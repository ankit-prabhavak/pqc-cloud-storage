...
export const deleteFile = async (req, res, next) => {
  try {
    ...
    await r2Client.send(
      new DeleteObjectCommand({ Bucket: BUCKET, Key: file.cloudUrl }),
    );
...
export const accessSharedFile = async (req, res, next) => {
  try {
    ...
    if (!file) {
      return res
        .status(404)
        .json({ message: "Shared link invalid or file not found" });
    }
