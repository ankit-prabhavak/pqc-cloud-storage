...
function VerifyOTPPage() {
  ...
  return (
    <main className="min-h-screen bg-white flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      ...
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        {/* Email icon */}
        <div style={{ width: 52, height: 52, background: '#f9fafb', border: '1.5px solid #e5e7eb', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24 }}>
          <FiMail size={22} color="#111827" />
        </div>

        {/* Heading */}
        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', letterSpacing: '-0.02em', marginBottom: 8 }}>
          Check your email
        </h1>
        ...
        {/* Success */}
        {success && (
          <div style={{ padding: '12px 16px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: '#15803d', margin: 0 }}>{success}</p>
          </div>
        )}

        {/* OTP inputs */}
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 24, justifyContent: 'space-between' }} onPaste={handlePaste}>
            {otp.map((digit, i) => (
              <input
                key={i}
                ...
                style={{
                  ...
                  fontFamily: 'DM Mono, monospace', color: '#111827',
                  border: `1.5px solid ${digit ? '#111827' : '#e5e7eb'}`,
                  borderRadius: 12, outline: 'none', background: digit ? '#f9fafb' : '#fff',
          ...
          >
            {loading ? 'Verifying...' : (<>Verify email <FiArrowRight size={15} /></>)}
          </button>
        </form>
          ...
          <p style={{ fontSize: 13, color: '#6b7280' }}>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 13, fontWeight: 600, color: countdown > 0 ? '#9ca3af' : '#111827'
              }}
            >
        ...
        {/* Back link */}
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid #f0f0f0', textAlign: 'center' }}>
          <Link href="/register" style={{ fontSize: 13, color: '#6b7280', textDecoration: 'none' }}>
